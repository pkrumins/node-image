...
module.exports.SyncImage = SyncImage;
function SyncImage (imageType, bufferType) {
    if (!(this instanceof SyncImage)) return new SyncImage(imageType, bufferType);
    if (!bufferType) bufferType = 'rgb';
    if (!libs[imageType]) libs[imageType] = require(imageType);
...
module.exports.SyncStack = SyncStack;
function SyncStack (imageType, bufferType) {
    if (!(this instanceof SyncStack)) return new SyncStack(imageType, bufferType);
    if (!bufferType) bufferType = 'rgb';
    if (!libs[imageType]) libs[imageType] = require(imageType);
